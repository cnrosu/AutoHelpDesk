...
$capturePlan = @(
  ...
  @{ Name = "Printing_Status"; Description = "Print spooler, queues, ports, drivers, and policies"; Action = {
      ...
      $collectorPath = Join-Path $repoRoot 'Collectors\Printing\Collect-PrintingStatus.ps1'
      if (Test-Path $collectorPath) {
        & $collectorPath
      } else {
        Write-Output ("CollectorMissing : {0}" -f $collectorPath)
      }
    } },
  @{ Name = "PrintService_Events"; Description = "PrintService event logs (last 7 days)"; Action = {
      $repoRoot = Split-Path -Parent $PSScriptRoot
      $collectorPath = Join-Path $repoRoot 'Collectors\Printing\Collect-PrintServiceEvents.ps1'
      if (Test-Path $collectorPath) {
        & $collectorPath -Days 7 -MaxEventsPerLog 200
      } else {
        Write-Output ("CollectorMissing : {0}" -f $collectorPath)
      }
    } },
