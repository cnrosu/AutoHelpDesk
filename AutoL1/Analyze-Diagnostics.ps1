...
if ($hasAdData) {
  ...
  $userDnsDomain = $null
  $logonServer = $null
  $domainCandidatesSet = New-Object 'System.Collections.Generic.HashSet[string]'
  $forestCandidatesSet = New-Object 'System.Collections.Generic.HashSet[string]'
  $domainEvidenceLines = @()
  ...
  $domainCandidates = $domainCandidatesSet.ToArray() | Sort-Object -Unique
  $forestCandidates = $forestCandidatesSet.ToArray() | Sort-Object -Unique
  ...
  if (-not $adDomainJoined) {
    ...
    foreach ($dcEntry in $discoveredDcEntries | Sort-Object Name) {
      $sourceText = ''
      if ($dcEntry.Sources -and $dcEntry.Sources.Count -gt 0) { $sourceText = ($dcEntry.Sources.ToArray() -join ', ') }
      $addrText = ''
      if ($dcEntry.Addresses -and $dcEntry.Addresses.Count -gt 0) { $addrText = ($dcEntry.Addresses.ToArray() -join ', ') }
    ...
    foreach ($dcEntry in $dcPortMap.Values | Sort-Object Name) {
      ...
      $addrText = ''
      if ($dcEntry.RemoteAddresses -and $dcEntry.RemoteAddresses.Count -gt 0) { $addrText = " Addr=" + ($dcEntry.RemoteAddresses.ToArray() -join ',') }
