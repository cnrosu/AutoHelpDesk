...
$printingAnalyzerPath = Join-Path (Split-Path -Parent $PSScriptRoot) 'Analyzers\Printing\Analyze-Printing.ps1'
if (Test-Path $printingAnalyzerPath) {
  . $printingAnalyzerPath
}
...
if (Get-Command Invoke-PrintingAnalysis -CommandType Function -ErrorAction SilentlyContinue) {
  Invoke-PrintingAnalysis -Summary $summary -PrintingStatus $printingStatus -PrintingEventRecords $printingEventRecords
}
