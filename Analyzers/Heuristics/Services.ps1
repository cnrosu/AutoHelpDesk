    ...
    if ($servicesArtifact) {
        ...
        if ($payload) {
            ...
            if ($payload.PSObject.Properties['Services']) {
                $serviceContainer = $payload.Services
            }

            $services = @()
            if ($serviceContainer) {
                if ($serviceContainer -is [System.Collections.IEnumerable] -and -not ($serviceContainer -is [string])) {
                    $services = @($serviceContainer)
                } elseif ($serviceContainer.PSObject.Properties['Items']) {
                    $items = $serviceContainer.Items
                    if ($items -is [System.Collections.IEnumerable] -and -not ($items -is [string])) {
                        $services = @($items)
                    } elseif ($items) {
                        $services = @($items)
                    }
                } elseif ($serviceContainer) {
                    $services = @($serviceContainer)
                }
            }

            $collectionErrors = @()
            if ($serviceContainer -and $serviceContainer.PSObject.Properties['Error'] -and $serviceContainer.Error) {
                $collectionErrors += [string]$serviceContainer.Error
            }
            if ($payload.PSObject.Properties['CollectionErrors']) {
                $errorsValue = $payload.CollectionErrors
                if ($errorsValue -is [System.Collections.IEnumerable] -and -not ($errorsValue -is [string])) {
                    $collectionErrors += ($errorsValue | Where-Object { $_ })
                } elseif ($errorsValue) {
                    $collectionErrors += $errorsValue
                }
            }
            ...
            if ($services.Count -gt 0) {
                ...
                if ($collectionErrors.Count -gt 0) {
                    Add-CategoryIssue -CategoryResult $result -Severity 'low' -Title 'Service inventory collection encountered errors' -Evidence ($collectionErrors -join "`n") -Subcategory 'Service Inventory'
                }
            } else {
                $evidence = if ($collectionErrors.Count -gt 0) { $collectionErrors -join "`n" } else { 'Service inventory payload missing.' }
