function Invoke-SystemWindowsSearchChecks {
    ...
    if ($errorCountValue -ge $errorThreshold) {
        ...
        $evidence = "Error events in last {0}h: {1}" -f $lookbackHours, $errorCountValue
        $data = @{ ErrorCount = $errorCountValue; LookbackHours = $lookbackHours }
        $remediation = @(
    ...
    if ($snapshot.Index -and $snapshot.Index.PSObject.Properties['PathExists'] -and $snapshot.Index.PathExists -eq $false) {
        ...
        $evidence = "Configured index path: {0}" -f (if ($snapshot.Index.PSObject.Properties['Path']) { [string]$snapshot.Index.Path } else { '(unknown)' })
        $data = @{ IndexPath = $snapshot.Index.Path }
        $remediation = @(
    ...
    if ($sizeBytes -ne $null -and $sizeBytes -gt $criticalCatalogThresholdBytes -and $driveFreePct -ne $null -and $driveFreePct -lt 5) {
        ...
        $evidence = "Catalog size: {0:N2} GB; Drive free: {1}%" -f ($sizeBytes / 1GB), $driveFreePct
        $data = @{ IndexSizeBytes = $sizeBytes; DriveFreePercent = $driveFreePct }
        $remediation = @(
    ...
    if ($urlsToCrawl -gt 5000 -and $crawlRate -lt 5) {
        ...
        $evidence = "URLs queued: {0}; Crawl rate: {1}" -f $urlsToCrawl, $crawlRate
        $data = @{ UrlsToCrawl = $urlsToCrawl; CrawlRate = $crawlRate }
        $remediation = @(
    ...
    if ($snapshot.Policy -and $snapshot.Policy.PSObject.Properties['PreventIndexingUserFolders'] -and $snapshot.Policy.PreventIndexingUserFolders -eq 1) {
        ...
        $evidence = 'Group Policy PreventIndexingUserFolders is enabled.'
        $data = @{ PreventIndexingUserFolders = 1 }
        $remediation = @(
    ...
    if ($sizeBytes -ne $null -and $sizeBytes -gt $largeCatalogThresholdBytes) {
        ...
        $evidence = "Catalog size: {0:N2} GB" -f ($sizeBytes / 1GB)
        $data = @{ IndexSizeBytes = $sizeBytes }
        $remediation = @(
