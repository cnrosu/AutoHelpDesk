function Add-SystemOperatingSystemInsights {
    param(
        [Parameter(Mandatory)]
        $Context,
    ...
    if ($systemArtifact) {
        ...
        if ($payload -and $payload.OperatingSystem -and -not $payload.OperatingSystem.Error) {
            ...
            if ($caption) {
                ...
                if ($captionLower -match 'windows\s+11') {
                    Add-CategoryNormal -CategoryResult $CategoryResult -Title ("Operating system supported: {0}" -f $description)
                } else {
                    ...
                    if ($unsupportedMatch.Success) {
                        ...
                        Add-CategoryIssue -CategoryResult $CategoryResult -Severity 'critical' -Title 'Operating system unsupported' -Evidence $evidence -Subcategory 'Operating System'
                    } else {
                        Add-CategoryCheck -CategoryResult $CategoryResult -Name 'Operating system' -Status $description
                    }
                }
            }
            if ($os.LastBootUpTime) {
                Add-CategoryCheck -CategoryResult $CategoryResult -Name 'Last boot time' -Status ([string]$os.LastBootUpTime)
            }
        } elseif ($payload -and $payload.OperatingSystem -and $payload.OperatingSystem.Error) {
            Add-CategoryIssue -CategoryResult $CategoryResult -Severity 'info' -Title 'Unable to read OS inventory' -Evidence ($payload.OperatingSystem.Error) -Subcategory 'Operating System'
        } else {
            Add-CategoryIssue -CategoryResult $CategoryResult -Severity 'info' -Title 'Operating system inventory not available' -Subcategory 'Operating System'
        }
        ...
        if ($payload -and $payload.ComputerSystem -and -not $payload.ComputerSystem.Error) {
            ...
            if ($cs.TotalPhysicalMemory) {
                ...
                Add-CategoryCheck -CategoryResult $CategoryResult -Name 'Physical memory (GB)' -Status ([string]$gb)
            }
        } elseif ($payload -and $payload.ComputerSystem -and $payload.ComputerSystem.Error) {
            Add-CategoryIssue -CategoryResult $CategoryResult -Severity 'info' -Title 'Unable to query computer system details' -Evidence $payload.ComputerSystem.Error -Subcategory 'Hardware Inventory'
        }
            ...
            if ($systemInfoText) {
                ...
                if ($biosModeMatch.Success) {
                    ...
                    if ($uefi -and -not $secureBootMatch.Success) {
                        ...
                        Add-CategoryIssue -CategoryResult $CategoryResult -Severity 'high' -Title 'Secure Boot state not reported despite UEFI firmware.' -Evidence (($evidence | Where-Object { $_ }) -join "`n") -Subcategory 'Firmware'
                    }
                }
            }
        }
    } else {
        Add-CategoryIssue -CategoryResult $CategoryResult -Severity 'info' -Title 'System inventory artifact missing' -Subcategory 'Collection'
    }
}
