...
ipconfig /registerdns"@
                ...
                Add-CategoryIssue -CategoryResult $result -Severity 'high' -Title ('DNS lookup failures: {0} — DNS resolution is failing.' -f ($names -join ', ')) -Evidence $evidence -Subcategory 'DNS Resolution' -Remediation $dnsResolutionRemediation -Data (& $createConnectivityData $connectivityContext)
            } else {
