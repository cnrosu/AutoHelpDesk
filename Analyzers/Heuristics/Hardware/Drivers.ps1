function Invoke-HardwareDriverHeuristic {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        $Context,
        ...
        [int]$StartingIssueCount = 0
    )

    $issueCount = $StartingIssueCount
    ...
    if (-not $driversArtifact) {
        ...
        return [pscustomobject]@{ IssueCount = ($issueCount + 1); ShouldReturn = $true }
    }
    ...
    Write-HeuristicDebug -Source 'Hardware' -Message 'Device Manager analysis completed' -Data ([ordered]@{
        IssuesRaised = ($issueCount - $StartingIssueCount)
    })

    if ($issueCount -eq 0) {
        Add-CategoryNormal -CategoryResult $CategoryResult -Title 'Device Manager reports all drivers healthy.' -Subcategory 'Device Manager'
    }
