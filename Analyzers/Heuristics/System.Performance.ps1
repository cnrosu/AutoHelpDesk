function Add-SystemPerformanceInsights {
    param(
        [Parameter(Mandatory)]
        $Context,
    ...
    if ($performanceArtifact) {
        ...
        if ($payload -and $payload.Memory -and -not $payload.Memory.Error) {
            ...
            if ($memory.TotalVisibleMemory -and $memory.FreePhysicalMemory) {
                ...
                if ($totalMb -gt 0) {
                    ...
                    Add-CategoryCheck -CategoryResult $CategoryResult -Name 'Memory utilization (%)' -Status ([string]$usedPct)
                    if ($usedPct -ge 90) {
                        Add-CategoryIssue -CategoryResult $CategoryResult -Severity 'medium' -Title 'High memory utilization detected' -Evidence ("Used memory percentage: {0}%" -f $usedPct) -Subcategory 'Performance'
                    }
                }
            }
        }
        ...
        if ($payload -and $payload.TopCpuProcesses) {
            if (($payload.TopCpuProcesses | Where-Object { $_.Error }).Count -gt 0) {
                Add-CategoryIssue -CategoryResult $CategoryResult -Severity 'info' -Title 'Unable to enumerate running processes' -Subcategory 'Performance'
            } else {
                $topProcess = $payload.TopCpuProcesses | Select-Object -First 1
                if ($topProcess -and $topProcess.CPU -gt 0) {
                    Add-CategoryCheck -CategoryResult $CategoryResult -Name 'Top CPU process' -Status ($topProcess.Name) -Details ("CPU time: {0}" -f $topProcess.CPU)
                }
            }
        }
    }
